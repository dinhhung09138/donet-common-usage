...
`:key directive` · `v-for reactivity caveats` · `array index anti-pattern` · `memoized list items (v-memo)` · `empty state` · `stable identity`

## Tasks

- [ ] Render a list of items with `v-for` using a stable unique `:key`
...
- [ ] Apply `v-memo` on list items and prove (via console logs) that unrelated updates don't re-render every item
