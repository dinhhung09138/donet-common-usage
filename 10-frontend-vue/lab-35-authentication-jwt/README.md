# Lab: Authentication & JWT Session Handling (Vue)
...
`JWT access/refresh token` · `axios interceptor token attach + refresh` · `navigation guard` · `storage strategy (httpOnly cookie vs memory)` · `storage/tab sync event`
...
- [ ] Build a global navigation guard (`router.beforeEach`) redirecting unauthenticated users to `/login?redirect=`
