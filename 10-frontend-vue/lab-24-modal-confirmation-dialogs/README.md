# Lab: Modal & Confirmation Dialogs (Vue)
...
`Teleport` · `focus trap` · `imperative confirm() promise pattern` · `aria-modal` · `scroll lock`

## Tasks

- [ ] Build a `Modal` component rendered via `<Teleport to="body">` with `aria-modal`, focus trap, and Escape-to-close
- [ ] Build a `confirm(options): Promise<boolean>` utility callable from any handler without template boilerplate
