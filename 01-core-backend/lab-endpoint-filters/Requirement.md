...
- [ ] Compare with MVC `ActionFilter`: write the equivalent `ValidationActionFilter : IActionFilter`; document the differences in README (IEndpointFilter works with Minimal API and is interface-based; ActionFilter is attribute-based and MVC-only)
- [ ] Write integration tests asserting that invalid input returns 422, a duplicate idempotency key returns the cached response, and `X-Response-Time-Ms` header is present
...
- [ ] Lab completed
- [ ] PR description written → `src/05-technical-english/pr-descriptions/lab-endpoint-filters-pr.md`
