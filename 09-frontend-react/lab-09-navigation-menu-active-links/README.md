# Lab: Navigation Menu & Active Link Highlighting (React)
...
`NavLink` · `isActive styling` · `config-driven menu` · `collapsible menu groups` · `mobile drawer` · `focus trap`

## Tasks

- [ ] Build a `menuConfig` array (label, path, icon, children) driving the rendered menu
- [ ] Use `NavLink` to auto-highlight the active top-level and nested item
- [ ] Implement collapsible menu groups (expand/collapse with animation)
