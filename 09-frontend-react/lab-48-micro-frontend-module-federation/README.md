# Lab: Micro-Frontend / Module Federation Basics (React)
...
`Module Federation (Webpack/Vite plugin)` · `host/remote split` · `shared singleton dependency` · `remote load failure fallback`

## Tasks

- [ ] Configure a `host` app and one `remote` app using Module Federation (`@originjs/vite-plugin-federation` or Webpack)
