# Lab: Error Boundary & Global Error Handling (React)
...
`componentDidCatch/ErrorBoundary` · `window.onerror/unhandledrejection` · `error reporting context (breadcrumbs)` · `fallback UI per boundary granularity`

## Tasks

- [ ] Build a reusable `ErrorBoundary` class component with a customizable fallback prop
- [ ] Wrap each major route/section in its own boundary so one crash doesn't blank the whole app
- [ ] Hook `window.onerror` and `unhandledrejection` to capture errors outside the React tree
