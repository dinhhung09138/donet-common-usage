# Lab: State Persistence & Offline Cache (React)
...
`persisted store (redux-persist or manual)` · `IndexedDB (idb)` · `stale-while-revalidate` · `navigator.onLine + online/offline events` · `offline mutation queue`

## Tasks

- [ ] Persist a slice of the store to `localStorage` with a version key and a migration function for schema changes
